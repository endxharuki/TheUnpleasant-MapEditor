...
ID3D11RenderTargetView* Renderer::m_SwapChainRenderTargetView{};
ID3D11Texture2D* Renderer::m_GameRenderTargetTexture{};
ID3D11Buffer* Renderer::m_SwapChainBuffer{};
ID3D11Texture2D* Renderer::m_RenderTargetTexture{};
ID3D11ShaderResourceView* Renderer::m_TextureSRV{};
...
void Renderer::Init()
{
	...
	m_SwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&m_SwapChainBuffer);

	m_Device->CreateRenderTargetView(m_SwapChainBuffer, NULL, &m_SwapChainRenderTargetView);
	//m_RenderTargetTexture->Release();

	
	// **シェーダーリソース用のテクスチャ（ゲーム画面をコピーする）**
	...
	renderTargetDesc.BindFlags = D3D11_BIND_RENDER_TARGET | D3D11_BIND_SHADER_RESOURCE; // シェーダーリソースとしても使う
	renderTargetDesc.CPUAccessFlags = 0;
	renderTargetDesc.MiscFlags = 0;

	m_Device->CreateTexture2D(&renderTargetDesc, NULL, &m_GameRenderTargetTexture);

	// **ゲーム画面用の RenderTargetView を作成**
	m_Device->CreateRenderTargetView(m_GameRenderTargetTexture, NULL, &m_RenderTargetView);
	

	// **ゲーム画面用の ShaderResourceView を作成**
	...
	m_Device->CreateShaderResourceView(m_GameRenderTargetTexture, &srvDesc, &m_TextureSRV);
...
void Renderer::Update()
{
	...
	// **ゲーム画面を `m_TextureSRV` にコピー**
	//m_DeviceContext->CopyResource(m_SwapChainBuffer, m_GameRenderTargetTexture);
	m_DeviceContext->OMSetRenderTargets(1, &m_SwapChainRenderTargetView, nullptr);
