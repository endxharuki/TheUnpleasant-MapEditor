...
void ModelRenderer::Draw()
{
	...
	/*m_Model->SubsetArray[2].Material.Material.TextureEnable = true;*/
}

void ModelRenderer::CreateBoneBuffer(ID3D11Device* device, int boneCount)
{

	// ボーン行列を格納するバッファを作成
	D3D11_BUFFER_DESC desc = {};
	desc.Usage = D3D11_USAGE_DEFAULT;
	desc.ByteWidth = sizeof(DirectX::XMMATRIX) * boneCount; // ボーン行列全体のサイズ
	desc.BindFlags = D3D11_BIND_SHADER_RESOURCE;
	desc.StructureByteStride = sizeof(DirectX::XMMATRIX); // 各要素のサイズ
	desc.MiscFlags = D3D11_RESOURCE_MISC_BUFFER_STRUCTURED;

	device->CreateBuffer(&desc, nullptr, &m_BoneBuffer);

	// シェーダーリソースビューを作成
	D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc = {};
	srvDesc.Format = DXGI_FORMAT_UNKNOWN; // 構造化バッファの場合は UNKNOWN
	srvDesc.ViewDimension = D3D11_SRV_DIMENSION_BUFFER;
	srvDesc.Buffer.ElementWidth = boneCount; // 要素数

	device->CreateShaderResourceView(m_BoneBuffer, &srvDesc, &m_BoneSRV);
